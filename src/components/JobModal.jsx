...
export default function JobModal({ job, customers, onSave, onDelete, onArchive, onRestore, onClose, settings, onStartTimer, activeTimer, timerStopKey }) {
  ...
  async function loadEntriesForUnit(unitId) {
    ...
    setUnitEntries(prev => ({ ...prev, [unitId]: entries }))
  }

  useEffect(() => {
    if (!job?.id) return
    units.forEach(u => { if (u.id) loadEntriesForUnit(u.id) })
  }, [job?.id, timerStopKey])
            ...
            <div className="space-y-3">
              {units.map((unit, idx) => (
                <div key={idx} className="border border-slate-200 rounded-xl p-3 space-y-2 bg-slate-50">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {unit.brand === 'Fox' && (<>
                        ...
                        <a href="/fox-fork-oil-volumes.html" target="_blank" rel="noopener noreferrer"
                          ...
                          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-slate-500" fill="currentColor"><path d="M12 2C9 7 6 10.5 6 14a6 6 0 0 0 12 0c0-3.5-3-7-6-12z"/></svg>
                        </a>
                        <a href="/fox-ifp-table.html" target="_blank" rel="noopener noreferrer"
                          className="w-5 h-5 rounded bg-slate-100 flex items-center justify-center shrink-0" title="Fox IFP Table">
                          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="10" width="18" height="4" rx="1"/><path d="M7 10V7a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v3"/><path d="M7 14v3a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-3"/><line x1="12" y1="6" x2="12" y2="4"/><line x1="12" y1="18" x2="12" y2="20"/></svg>
                        </a>
                      </>)}
                      {unit.brand === 'Rockshox' && (<>
                        ...
                        <a href="/rockshox-fork-oil-volumes.html" target="_blank" rel="noopener noreferrer"
                          className="w-5 h-5 rounded bg-slate-100 flex items-center justify-center shrink-0" title="Rockshox Oil Volumes">
                          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-slate-500" fill="currentColor"><path d="M12 2C9 7 6 10.5 6 14a6 6 0 0 0 12 0c0-3.5-3-7-6-12z"/></svg>
                        </a>
                        <a href="/rockshox-ifp-table.html" target="_blank" rel="noopener noreferrer"
                          className="w-5 h-5 rounded bg-slate-100 flex items-center justify-center shrink-0" title="Rockshox IFP Table">
                          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="10" width="18" height="4" rx="1"/><path d="M7 10V7a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v3"/><path d="M7 14v3a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-3"/><line x1="12" y1="6" x2="12" y2="4"/><line x1="12" y1="18" x2="12" y2="20"/></svg>
                        </a>
                      </>)}
                    </div>
                    {units.length > 1 && (
                      pendingConfirm?.key === `unit-${idx}` ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => setPendingConfirm(null)} className="text-xs text-slate-400 font-medium">Cancel</button>
                          <button onClick={() => { removeUnit(idx); setPendingConfirm(null) }} className="text-xs text-white bg-red-500 font-semibold px-2 py-0.5 rounded-md">Remove</button>
                        </div>
                      ) : (
                        <button onClick={() => setPendingConfirm({ key: `unit-${idx}` })} className="text-red-400 text-xs font-medium">Remove</button>
                      )
                    )}
                  </div>
                  ...
                  {/* Per-unit time tracking — only for saved units */}
                  {!isNew && unit.id && onStartTimer && (() => {
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-colors ${
                              isRunning
                                ...
                                : 'bg-white text-slate-600 border-slate-300 active:bg-slate-50'
                            }`}
                          >
                            {isRunning
                              ? <><span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse block" /> Recording</>
                              : <><svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Record</>
                            }
                          </button>
                        </div>
                                  ...
                                  <div className="flex items-center gap-1 mt-0.5">
                                    {['h', 'm', 's'].map(key => (
                                      <div key={key} className="flex items-center gap-0.5">
                                        <input type="number" min="0" max={key === 'h' ? 99 : 59}
                                          value={editingEntry[key]}
                                          onChange={e => setEditingEntry(ed => ({ ...ed, [key]: Math.max(0, Math.min(key === 'h' ? 99 : 59, Number(e.target.value))) }))}
                                          className="w-9 text-center border border-slate-300 rounded text-xs py-0.5" />
                                    ...
                                    <button onClick={async () => {
                                      ...
                                      loadEntriesForUnit(unit.id)
                                    }} className="ml-1 text-[10px] font-bold text-sky-600 px-2 py-0.5 bg-sky-50 rounded">Save</button>
                                    <button onClick={() => setEditingEntry(null)} className="text-[10px] text-slate-400 px-1">✕</button>
                                  </div>
                                ) : (
                                  <p className="text-xs font-mono font-bold text-slate-700">
                                    {entry.duration_seconds ? formatHMS(entry.duration_seconds) : <span className="text-sky-400 animate-pulse text-[10px]">Running…</span>}
                                  </p>
                                )}
                              </div>
                              {!isEditing && entry.duration_seconds && (
                                pendingConfirm?.key === `entry-${entry.id}` ? (
                                  <div className="flex items-center gap-2 shrink-0">
                                    <span className="text-[10px] text-red-500 font-medium">Sure?</span>
                                    <button onClick={() => setPendingConfirm(null)} className="text-[10px] text-slate-400 font-medium">Cancel</button>
                                    <button onClick={async () => {
                                      await deleteEntry(entry.id)
                                      setPendingConfirm(null)
                                      loadEntriesForUnit(unit.id)
                                    }} className="text-[10px] text-white bg-red-500 font-semibold px-2 py-0.5 rounded-md">Delete</button>
                                  </div>
                                ) : (
                                  <div className="flex gap-0.5 shrink-0">
                                    ...
                                    }} className="p-1.5 text-slate-300 active:text-slate-600">
                                      <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487z" /></svg>
                                    </button>
                                    <button onClick={() => setPendingConfirm({ key: `entry-${entry.id}` })} className="p-1.5 text-slate-300 active:text-red-500">
