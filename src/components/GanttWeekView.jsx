...
export default function GanttWeekView({ jobs, onJobClick, viewMode }) {
  ...
  function barBounds(job) {
    ...
    const left = differenceInDays(s, anchor) * COL_W + 2
    const width = (differenceInDays(e, s) + 1) * COL_W - 4
    return { left, width }
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto scrollbar-none">
        <div className="flex min-h-full">
          ...
          >
            {/* Header cell */}
            <div className="h-10 border-b border-slate-200 flex items-center px-2">
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(job => (
                <button
                  ...
                  className="border-b border-slate-100 flex items-center px-2 text-left w-full hover:bg-sky-50 transition-colors"
                >
          ...
          {/* Scrollable chart */}
          <div className="flex-1 overflow-x-auto scrollbar-none" style={{ minWidth: days.length * COL_W }}>
            {/* Day headers */}
            <div
              className="sticky top-0 z-10 flex bg-white border-b border-slate-200"
              style={{ height: 40, width: days.length * COL_W }}
            >
              {days.map(day => (
                <div
                  key={day.toISOString()}
